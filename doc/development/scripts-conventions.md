...
- Project code never reads or writes anywhere under `.git/`. That directory is git's internal state, subject to git's own locking and conventions; any project write concurrent with a git operation on a shared `.git/` (worktrees) risks lock errors, and the directory is read-only in sandboxed agent environments (e.g. Codex). File exclusions go through a versioned `.gitignore` at the repository root or a `.gitignore` placed in the relevant subdirectory — never through `.git/info/exclude` or any other file under `.git/`. Helpers like `git rev-parse --git-path` or `git update-index --assume-unchanged` exist only to coordinate with git internals and have no place in project tooling. The pre-commit hook is versioned under `scripts/githooks/pre-commit`. Running `php scripts/scripts-install.php` sets `core.hooksPath = scripts/githooks` once in the WP git config; linked worktrees (WAs) inherit it automatically. The hook is never copied at runtime and is never placed in `.git/hooks/`.
...
- **Backlog path centralization**: all paths to backlog files (`backlog-board.yaml`, `backlog-review.md`, `local/backlog-review-result.txt`) and the `local/backlog/` directory are defined exclusively in `Sowapps\SoManAgent\Script\Backlog\BacklogPaths`. No literal path string such as `'local/backlog/backlog-board.yaml'` may appear outside that class — use `BacklogPaths::boardPath($root)`, `BacklogPaths::reviewPath($root)`, `BacklogPaths::directory($root)`, or `BacklogPaths::BOARD` / `BacklogPaths::DIRECTORY` constants. This rule applies to both production code and tests; any path concatenated inline is a bug.
