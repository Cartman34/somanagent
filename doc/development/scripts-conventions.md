...
- **Backlog path centralization**: all paths to backlog files (`backlog-board.yaml`, `backlog-review.md`, `local/backlog-review-result.txt`) and the `local/backlog/` directory are defined exclusively in `SoManAgent\Script\Backlog\BacklogPaths`. No literal path string such as `'local/backlog/backlog-board.yaml'` may appear outside that class — use `BacklogPaths::boardPath($root)`, `BacklogPaths::reviewPath($root)`, `BacklogPaths::directory($root)`, or `BacklogPaths::BOARD` / `BacklogPaths::DIRECTORY` constants. This rule applies to both production code and tests; any path concatenated inline is a bug.
