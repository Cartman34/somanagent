...
- run `SOMANAGER_ROLE=manager SOMANAGER_AGENT=<code> php scripts/backlog/backlog.php ...` from `WP` only; backlog commands are not allowed from `WA`
...
- bypass `php scripts/backlog/backlog.php` when it already covers the needed backlog action
...
- Every manager backlog command must be prefixed exactly as `SOMANAGER_ROLE=manager SOMANAGER_AGENT=<code> php scripts/backlog/backlog.php ...`.
...
The config is read from `backlog/config.yaml` (key `workflow.submit`). This file is generated by `php scripts/backlog/install.php` from the backlog package template, which ships with `workflow.submit: agent`. When the key is absent, the file is unreadable, or the value is unknown, the fallback is `user`. The operator can override the project config for a single session with `--submit-mode=user|agent|default` on `php scripts/backlog/agent.php start`.

### `scopes`

Declares named scope constraints that restrict which directories an entry may touch.
...
Each key is a scope name; each value is a list of directory prefixes (relative to the project root). A file is in scope when its path starts with at least one listed prefix. The name `ALL` is reserved and must not be used. When the `scopes:` section is absent, no scope enforcement applies.

Scope names are passed to `entry-create --scope=<name>`. The mechanical review (`review-check`) enforces the declared directories for every entry that carries a scope. An entry with no `scope:` field is unrestricted (ALL).
