...
The config is read from `local/backlog/config.yaml` (key `workflow.submit`). This file is generated by `php scripts/setup.php install` from `scripts/resources/backlog/config.yaml.dist`, which ships with `workflow.submit: agent`. When the key is absent, the file is unreadable, or the value is unknown, the fallback is `user`. The operator can override the project config for a single session with `--submit-mode=user|agent|default` on `php scripts/backlog-agent.php start`.
