...
15. The unified commands `review-check`, `review-approve`, and `review-reject` handle both `kind=feature` and `kind=task` entries. For `kind=task` entries, review notes are stored under `local/backlog-review.md` with keys shaped as `<feature>/<task>`.
