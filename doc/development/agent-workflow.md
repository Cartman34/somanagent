...
7a. `list --stage=todo` prints queued tasks, one per line with `ref=` as the stable reference and the only valid target for mutating commands (`task-remove`, `work-start`). `list` is read-only and never mutates backlog state.
