...
7a. `list --stage=todo` prints queued tasks, one per line with the stable ref printed first (no `ref=` label), followed by `kind=`, `agent=`, `pr=`, `reviewer=`, and `title=` fields. The ref is the only valid target for mutating commands (`task-remove`, `work-start`). `list` is read-only and never mutates backlog state.
