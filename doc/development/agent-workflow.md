...
4. Developer and reviewer commands on `backlog.php` identify the caller via the `SOMANAGER_AGENT` environment variable. `--agent` is no longer passed on those commands.
...
15a. `SOMANAGER_AGENT=<reviewer> php scripts/backlog.php review-next [<feature>|<feature/task>]` selects an entry in `review`, transitions it to `reviewing`, records the reviewer in `meta.reviewer`, and displays the entry. Without a target, it picks the first entry in `review`. With an explicit reference (same shape as `review-list`), it claims that exact entry and refuses when the entry is already in `reviewing` (claimed) or no longer in `review`. In all cases it refuses when the reviewer already has an entry in `reviewing`; entries already in `reviewing` are not picked implicitly.
15b. `SOMANAGER_AGENT=<reviewer> php scripts/backlog.php review-cancel <feature|feature/task>` moves the reviewer's `reviewing` entry back to `review` and clears `meta.reviewer`. The reference is mandatory: review-cancel never auto-resolves the entry from the agent code, so the mutation cannot silently retarget another claim. The stored reviewer is verified against `SOMANAGER_AGENT` before the board is saved; a manager (`SOMANAGER_ROLE=manager`, combined with `SOMANAGER_AGENT=<manager>`) may force-cancel any stuck reviewing entry with the same explicit reference contract.
...
15d. `SOMANAGER_AGENT=<reviewer> php scripts/backlog.php review-check <feature|feature/task>`, `review-approve <feature|feature/task>`, and `review-reject <feature|feature/task> --body-file=<path>` accept both feature references and `<feature/task>` references; the command delegates internally based on whether the reference contains `/`. Short task references (bare task slug without the parent feature) are refused. These are the only public commands for the reviewer review flow; the underlying `feature-review-*` and `task-review-*` forms are no longer public.
15e. `review-list` prints entries waiting in `review`, one per line shaped `- <ref> kind=<feature|task> agent=<x> ...`, where `<ref>` is the stable reference usable by `review-next`. Entries already in `reviewing` are excluded. The command is read-only and never mutates backlog state.
16. `SOMANAGER_AGENT=<code> php scripts/backlog.php rework [<feature>|<task>|<feature/task>]` moves one rejected or approved task or feature back to `development` and reopens the entry branch in that agent worktree. It displays the stored review notes for rejected entries, and is also the recovery path when `entry-merge` aborts on a conflict on an approved entry. The command leaves the existing GitHub PR untouched.
...
19. `SOMANAGER_AGENT=<agent> php scripts/backlog.php entry-merge <feature/task>` merges one child task branch into its parent feature branch locally, after a green mechanical review in the task worktree, using either the worktree already bound to the parent branch or a temporary merge worktree.
20. `SOMANAGER_AGENT=<reviewer> php scripts/backlog.php entry-merge <feature>` is the recommended reviewer form for merging a feature into `main`; it prints the resolved type, target, merge target, and equivalent internal command before delegating to `feature-merge`.
21. `SOMANAGER_AGENT=<reviewer> php scripts/backlog.php entry-merge <feature/task>` is the recommended reviewer form for merging one explicit child task locally; `SOMANAGER_AGENT` identifies the reviewer calling the command and is never used as a developer task owner.
22. `entry-merge <task>` is refused even when the task slug is unique, because task merges must use the full `<feature/task>` reference.
23. `SOMANAGER_AGENT=<code> php scripts/backlog.php entry-merge <feature/task>` is the form for merging a task after an explicit user merge instruction; `<code>` is the calling agent code (reviewer or developer).
