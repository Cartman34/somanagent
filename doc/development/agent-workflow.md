...
23. `feature-task-merge --agent=<code> [<task>]` is the developer form for merging the current agent task after an explicit user merge instruction.
24. `feature-task-merge <feature/task>` and `feature-merge <feature>` stay available as compatible lower-level commands, but reviewers should prefer `entry-merge`.
