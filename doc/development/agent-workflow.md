...
- supports `--loop` only with `--watch`. A clean client exit prints `Task done — waiting for next.` followed by a blank line, then returns to the watch loop with no role or code carried over from the previous cycle. A non-zero client exit stops the loop and returns that error.
