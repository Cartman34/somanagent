...
3. For `entry-unassign`, `--developer` identifies the caller developer. With an explicit entry reference, it does not select which assigned developer is removed.
...
5. `--developer` is used by `entry-assign`, `entry-unassign`, and `worktree-restore` to identify the target developer. `--agent` is kept for role-agnostic commands: `status` and `review-notes`.
...
7a. `list --stage=todo` prints queued tasks, one per line with the stable ref printed first (no `ref=` label), followed by `kind=`, `developer=`, `pr=`, `reviewer=`, and `title=` fields. The ref is the only valid target for mutating commands (`task-remove`, `work-start`). `list` is read-only and never mutates backlog state.
...
9. `entry-release` returns the active feature or task to `todo:` only when no development was done on its branch. A parent `kind=feature` cannot be released while child `kind=task` entries still exist for that feature. Developer and reviewer callers can omit `<entry-ref>` to target their own active entry; manager callers must pass an explicit `<entry-ref>`.
...
23. `SOMANAGER_ROLE=<role> SOMANAGER_AGENT=<code> php scripts/backlog.php entry-merge <entry-ref>` is the form for merging a task after an explicit user merge instruction; `<role>` is the caller role and `<code>` is the calling agent code. The output prints `Reviewer:` from the entry metadata and a separate `Caller:` line from the active environment.
