...
5. Prefix the description with `[type:feat]` or `[type:fix]` so `feature-start` can derive the branch type from the backlog entry.
...
2. Prefix the description with `[type:feat]` or `[type:fix]`.
