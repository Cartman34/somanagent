...
| `backend/.env.local` | No (gitignored) | Local Symfony overrides (non-Docker use, generated by the WA workflow) |
| `scripts/resources/backlog/config.yaml.dist` | Yes | Backlog config template — materialized to `local/backlog/config.yaml` by `setup.php install` |
| `local/backlog/config.yaml` | No (gitignored) | Local backlog settings; created from the `.dist` on first install, never overwritten on subsequent installs |

### Backlog local config pattern

`scripts/resources/backlog/config.yaml.dist` is the committed template and defines all supported keys with their default values. It is the canonical reference for available settings.

`local/backlog/config.yaml` is the per-workstation copy. `php scripts/setup.php install` (and `update`) creates it from the `.dist` if it does not already exist. Existing local files are never overwritten, so local customizations are preserved across updates.
