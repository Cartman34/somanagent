...
| `backend/.env.local` | No (gitignored) | Local Symfony overrides (non-Docker use, generated by the Agent Worktree workflow) |
| `backlog/config.yaml` | No (gitignored) | Local backlog settings, managed by the backlog package; created by `scripts/backlog/install.php`, never overwritten on subsequent installs |
