...
const TOKENS_PAGE_TRANSLATION_KEYS = [
  ...
  'token.card.input',
  'token.card.output',
  'token.card.totalCalls',
  'token.section.byAgent',
  'token.empty.noConsumption',
  'token.table.agent',
  'token.table.input',
  'token.table.output',
  'token.table.total',
...
export default function TokensPage() {
  ...
  return (
    <>
      <PageHeader title={t('token.page.title')} description={t('token.page.description')}
      ...
      <div className="relative">
        ...
        {[
          { label: t('token.card.input'),  value: total?.input  ?? 0 },
          { label: t('token.card.output'),  value: total?.output ?? 0 },
          { label: t('token.card.totalCalls'),  value: total?.calls  ?? 0 },
        ].map(({ label, value }) => (
      ...
      {/* Par agent */}
      <h2 className="text-base font-semibold text-gray-900 mb-3">{t('token.section.byAgent')}</h2>

      {!summary?.byAgent?.length ? (
        <div className="card p-8 text-center">
          <Coins className="w-10 h-10 mx-auto mb-3 text-gray-300" />
          <p className="text-sm text-gray-400">{t('token.empty.noConsumption')}</p>
        </div>
      ) : (
        <div className="list-agent card overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-gray-600">{t('token.table.agent')}</th>
                <th className="text-right px-4 py-3 font-medium text-gray-600">{t('token.table.input')}</th>
                <th className="text-right px-4 py-3 font-medium text-gray-600">{t('token.table.output')}</th>
                <th className="text-right px-4 py-3 font-medium text-gray-600">{t('token.table.total')}</th>
                <th className="text-right px-4 py-3 font-medium text-gray-600">{t('token.table.calls')}</th>
              </tr>
            </thead>
